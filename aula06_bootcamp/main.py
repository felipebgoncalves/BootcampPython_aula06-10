...
def conexao_FTP():
    ...
    ftp.login(
        user=os.environ.get("USER_FTP"),
        passwd=os.environ.get("PASSWD_FTP"),
        acct="",
    )
    ftp.encoding = "utf-8"
    ...
    for pasta in pastas:
        ...
        for EMA in sorted(ftp.nlst()):
            ...
            try:
                ...
                # filename = EMA + '_1h_' + datetime.utcnow().strftime('%Y%m%d')
                # + '_' + hora_minuto + '.txt'
                # filename = EMA + '_24h_' + datetime.utcnow().strftime('%Y%m%d')
                # + '_' + hora_minuto + '.txt'
                ...
                print("Download e Inserção dos Dados Concluido!")
